...
const NEEDEDTOUPDATE int = 30000000
...
func main() {
	...
	// fmt.Printf()
	fmt.Printf("Size of %s: %d\n", thisOs.PWD[0].name, thisOs.PWD[0].Size())

	// get current free space:
	currFreeSpace := TOTALDISKSIZE - thisOs.PWD[0].Size()
	deltaToUpdate := NEEDEDTOUPDATE - currFreeSpace

	var min *FileObject

	for _, v := range thisOs.PWD[1:] {
		if v.Size() > deltaToUpdate {
			if min == nil {
				min = v
			} else if min.Size() > v.Size() {
				min = v
			}
		}
		// fmt.Printf("Size of %s: %d\n", v.name, v.Size())
	...
	fmt.Printf("Delete the %s directory to free up %d space!\n", min.name, min.Size())
